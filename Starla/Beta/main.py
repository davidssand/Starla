...
class Rocket:
  ...
  def set_events(self):
    ...
    # Declares all system events

    # --------------- #

    self.apogee = threading.Event()

  def change_checker(self, valid_value, operator, validation_time):
    # --------------- #

    # Checks for valid change in input variable for 
    # time greater than validation_time
  ...
  def main(self):
    ...
    loop_time = 0
    while not self.apogee.is_set():
      self.populate_data_arrays()

      if not self.apogee.is_set():
